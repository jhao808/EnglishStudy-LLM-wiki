---
name: query
description: 在 EnglishStudy Wiki 中回答英文学习问题、整理主题表达、解释用法，并按时间范围生成复习题。必须先读取 wiki/index.md，再读取相关 source、vocabulary、expressions、review 或 syntheses 页面；回答使用 [[wikilink]] 标注本地来源。
user-invocable: true
---
...
## 核心目标
将用户的问题转化为对本地 EnglishStudy Wiki 的检索与学习输出。默认用户水平为 **IELTS 7**，回答应重视主题表达、短语搭配、句子级表达、日常口语和写作迁移，而不是孤立单词释义。
...
- 所有解释用简体中文；英语表达、例句、题目和答案保留英文。
- 引用本地 wiki 信息时使用 `[[页面名称]]`。
...
- **口语输出**：生成日常对话、IELTS Part 1/2/3 回答素材、自然回应。
...
如果生成或更新了 review、synthesis、vocabulary 或 expressions 页面，必须追加：
```markdown
